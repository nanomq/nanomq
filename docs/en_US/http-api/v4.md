...
| --------------------------------- | ------------- | ------------------------------------------------------------ |
| code                              | Integer       | 0                                                            |
| data.basic.url                    | String        | Url of listener.                                             |
| data.basic.num_taskq_thread       | Integer       | Number of taskq threads used.                                |
| data.basic.max_taskq_thread       | Integer       | Maximum number of taskq threads used。                       |
| data.basic.parallel               | Long          | Number of parallel.                                          |
| data.basic.property_size          | Integer       | Max size for a MQTT property.                                |
| data.basic.msq_len                | Integer       | Queue length for resending messages.                         |
| data.basic.qos_duration           | Integer       | The interval of the qos timer.                               |
| data.basic.allow_anonymous        | Boolean       | Allow anonymous login.                                       |
...
### GET /api/v4/configuration/{CONFIG_NAME}

`CONFIG_NAME` Can be set as `basic` ,`tls` ,`sqlite` ,`auth` ,`auth_http` ,`websocket` ,`http_server` ,`bridge` ,`aws_bridge`;
...
```bash
$ curl -i --basic -u admin:public -X GET 'http://127.0.0.1:8081/api/v4/configuration/basic' 

{"code":0,"data":{"basic":{"url":"nmq-tcp://0.0.0.0:1883","num_taskq_thread":16,"max_taskq_thread":16,"parallel":16,"property_size":32,"daemon":false,"max_packet_size":1,"client_max_packet_size":1,"msq_len":2048,"qos_duration":10,"keepalive_backoff":1250,"allow_anonymous":true}}}%
```

 Get `TLS` Configuration:

```bash
$ curl -i --basic -u admin:public -X GET 'http://127.0.0.1:8081/api/v4/configuration/tls'  

{"code":0,"data":{"tls":{"enable":false,"url":"tls+nmq-tcp://0.0.0.0:8883","key_password":null,"key":null,"cert":null,"cacert":null,"verify_peer":false,"fail_if_no_peer_cert":false}}}% 
```
...
> Only take effect for the parameters which marked "Hot updatable" in the configuration file.
...
### POST /api/v4/configuration/{CONFIG_NAME}
...
```bash
$ curl -i --basic -u admin:public -X POST 'http://localhost:8081/api/v4/configuration/basic' -d \
'{
    "data": {
        "basic": {
            "url": "nmq-tcp://0.0.0.0:1883",
