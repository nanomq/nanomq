...
---------------------------------- | ------------- | ----------------------------------------------------------------------------------------------
system.daemon                      | Boolean       | Launch with daemon（_默认false_）。
...
bridge.mqtt.nodes[0].connector.conn_properties | Object        | MQTT V5 Property of  Connector (see table below) 
bridge.mqtt.nodes[0].connector.ssl.enable | Boolean       | Launch TLS （ _default false_）。                            
 bridge.mqtt.nodes[0].connector.ssl.key_password | String        | String containing the user's password. only used if the private keyfile is password-protected. 
 bridge.mqtt.nodes[0].connector.ssl.keyfile      | String        | User's private PEM-encoded key.                              
 bridge.mqtt.nodes[0].connector.ssl.certfile     | String        | User certificate data.                                       
 bridge.mqtt.nodes[0].connector.ssl.cacertfile   | String        | User's PEM-encoded CA certificates.                          
...
bridge.mqtt.nodes[0].hybrid_bridging        | Boolean       | Hybrid bridging: enable or disable the hybrid bridging mode，(default: `false`), recommend to enable it when you want to take advantage of QUIC 
bridge.mqtt.nodes[0].congestion_control     | String        | Congestion control option (cubic \| bbr) (default `bbr`) 
...
bridge.mqtt.nodes[0].sub_properties | Object        | MQTT V5 Property of Subscription (see table below) 

##### MQTT V5 Property 

`Connector`property: `bridge.mqtt.nodes[0].connector.conn_properties`
...
`Subscription` property :` bridge.mqtt.nodes[0].sub_properties`
