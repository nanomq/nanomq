...
bridges.mqtt.name.max_parallel_processes    | Long          | Parallel of mqtt client.
...
**📢Attention:** When multiple different bridging configurations are required, they can be differentiated by specifying different names. In this case, "cache" is shared by multiple MQTT bridging configurations. If message caching is required, as in the following example, add the cache configuration section to your configuration.
...
bridge.aws.nodes[0].forwards              | Array[String] | Array of forward topics.
...
HTTP authentication plugin only works for CONNECT, PUBLISH & SUBSCRIBE is not implemented yet.
