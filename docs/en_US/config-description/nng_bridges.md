...
* **Workflow**: MQTT Client publishes to `local_topic` → Bridge subscribes and extracts the payload → Payload is prefixed with `remote_topic` → Sent to NNG pub socket.
...
* `forwards`: Array of mapping rules between local MQTT topics and remote NNG topics.
    ...
    * `remote_topic`: The topic prefix prepended to the payload when sending to the NNG side. The constructed NNG message format is: `"remote_topic/payload"` (separated by `/`). If left empty, the original MQTT topic is used as the NNG message prefix.
...
* **Workflow**: NNG pub socket sends message with prefix → Bridge matches `remote_topic` prefix → Forwards remainder of message as MQTT payload to `local_topic`.
...
* `subscription`: Array of mapping rules between remote NNG topics and local MQTT topics.
    * `remote_topic`: The NNG topic prefix to subscribe to. The adapter uses prefix matching; if a message starts with `remote_topic/`, the content after the prefix is treated as the payload.
