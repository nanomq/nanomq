...
Query parquet records by topic and time range.

**Parameters (query):**

| Name     | Type    | Required | Description |
| -------- | ------- | -------- | ----------- |
| topic    | String  | True     | Exchange topic name. |
| start_ts | Integer | True     | Start timestamp (inclusive), unit: ms. |
| end_ts   | Integer | True     | End timestamp (inclusive), unit: ms. |
| schema   | String  | False    | Comma-separated column names (for example: `data,can_id`). If omitted, all non-ts columns are used. |
| limit    | Integer | False    | Maximum rows to return. If omitted, default is `10000`. `limit=0` is treated as invalid parameter and returns HTTP `400`. |
| offset   | Integer | False    | Number of matched rows to skip before returning data. Default is `0`. |
...
| dataArray           | Array of Objects | Query results. |
| dataArray[0].ts     | Integer          | Timestamp (ms). |
| dataArray[0].data   | String           | Base64 encoded payload for selected columns. |
...
```bash
$ curl -i --basic -u admin:public -X GET "http://localhost:8081/api/v4/mqtt_stream?topic=canudp&start_ts=1776308915923&end_ts=1776308918397"

{"count":1191,"dataArray":[{"ts":1776308915923,"data":"YWFhYWFhYWFhYWFhYWFhYQ=="}]}
```

Query a specific schema column:

```bash
$ curl -i --basic -u admin:public -G "http://localhost:8081/api/v4/mqtt_stream" \
    --data-urlencode "topic=canudp" \
    --data-urlencode "start_ts=1776308915923" \
    --data-urlencode "end_ts=1776308915940" \
    --data-urlencode "schema=data"


{"count":8,"dataArray":[{"ts":1776308915923,"data":"YWFhYWFhYWFhYWFhYWFhYQ=="},{"ts":1776308915924,"data":"YWFhYWFhYWFhYWFhYWFhYQ=="},{"ts":1776308915925,"data":"YWFhYWFhYWFhYWFhYWFhYQ=="},{"ts":1776308915926,"data":"YWFhYWFhYWFhYWFhYWFhYQ=="},{"ts":1776308915927,"data":"YWFhYWFhYWFhYWFhYWFhYQ=="},{"ts":1776308915931,"data":"YWFhYWFhYWFhYWFhYWFhYQ=="},{"ts":1776308915933,"data":"YWFhYWFhYWFhYWFhYWFhYQ=="},{"ts":1776308915940,"data":"YWFhYWFhYWFhYWFhYWFhYQ=="}]}
```
...
$ curl -i --basic -u admin:public -G "http://localhost:8081/api/v4/mqtt_stream" \
    --data-urlencode "topic=canudp" \
    --data-urlencode "start_ts=1776308915923" \
    --data-urlencode "end_ts=1776397489303" \
    --data-urlencode "limit=1000" \
    --data-urlencode "offset=2000"

{"count":1000,"dataArray":[{"ts":1776308919xxx,"data":"..."}]}
```
...
| ---- | ------- | ----------- |
| code | Integer | 0           |
| code | Integer | 115 No such bridging config          |
| code | Integer | 108 Bridge is already on           |
| code | Integer | 109 Request format wrong           |
