...
tls.enable                      | Boolean       | 启动TLS监听（_默认false_）。
...
websocket.enable                | Boolean       | 启动websocket监听（_默认true_）。
...
bridge.mqtt.nodes[0].clientid              | String        | 桥接客户端ID（_默认NULL为自动生成随机ID_）。
...
bridge.mqtt.nodes[0].tls.enable            | Boolean       | 启动TLS监听（_默认false_）。
...
bridge.aws.nodes[0].clientid              | String        | 桥接客户端ID（_默认NULL为自动生成随机ID_）。
