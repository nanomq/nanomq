...
bridges.mqtt.nodes[0].qsend_idle_timeout | Duration | QUIC传输层重置拥塞控制算法的等待超时时间 (*默认`60 s`*) 
bridges.mqtt.nodes[0].qinitial_rtt_ms | Duration | 初始 RTT 估计时间 (*默认 `800ms`*) 
bridges.mqtt.nodes[0].qmax_ack_delay_ms | Duration | 发送 ACK 之前接收数据后等待时长(默认`100ms`) 
