...
bridge.mqtt.nodes[0].connector.conn_properties | Object | Connector的 MQTT V5属性(见下表) 
bridge.mqtt.nodes[0].connector.ssl.enable | Boolean       | 启动TLS监听（_默认false_）。                                 
 bridge.mqtt.nodes[0].connector.ssl.key_password | String        | TLS私钥密码。                                                
 bridge.mqtt.nodes[0].connector.ssl.keyfile      | String        | TLS私钥数据。                                                
 bridge.mqtt.nodes[0].connector.ssl.certfile     | String        | TLS Cert证书数据。                                           
 bridge.mqtt.nodes[0].connector.ssl.cacertfile   | String        | TLS CA证书数据。 
...
bridge.mqtt.nodes[0].congestion_control     | String        | 拥塞控制选项 (cubic \| bbr) (_默认_ `bbr`)
...
bridge.mqtt.nodes[0].sub_properties | Object | Subscription的MQTT V5属性(见下表) 

##### MQTT V5 属性配置参数

`Connector`属性:`bridge.mqtt.nodes[0].connector.conn_properties`
...
`Subscription`属性: `bridge.mqtt.nodes[0].sub_properties`
