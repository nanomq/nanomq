...
| bridge.mqtt.{NAME}.congestion_control         | String        | 拥塞控制选项 (cubic \| bbr) (_默认_ `bbr`)                   |
| bridge.mqtt.{NAME}.multi_stream               | Boolean       | Multiple stream 开关，（_默认`false`不启用_）                |
| bridge.mqtt.{NAME}.parallel                   | Long          | 桥接客户端并发数。                                           |
| bridge.mqtt.{NAME}.forwards                   | Array[String] | 转发Topic数组, 使用逗号`,`分隔多个`Topic`。                  |
| bridge.mqtt.{NAME}.subscription[0].topic      | String        | 第1个订阅`Topic`。                                           |
| bridge.mqtt.{NAME}.subscription[0].qos        | Integer       | 第1个订阅`Qos`。                                             |
