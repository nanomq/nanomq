...
| data.bridges.nodes[0].subscription[0].topic | String        | 第 1 个订阅`Topic`。                                           |
...
此 API 允许用户动态更新桥接配置，而无需重启 NanoMQ，但会触发桥接重连。
...
| data.connector.clientid         | String        | 桥接客户端 ID （_默认 NULL 为自动生成随机 ID_）。              |
...
| data.connector.conn_properties  | Object        | Connector 的 MQTT V5 属性                                      |
| data.connector.will_properties  | Object        | MQTT V5 遗嘱属性                                              |
| data.connector.ssl.enable       | Boolean       | 启动 TLS 监听（*默认`false`*）。                               |
| data.connector.ssl.key_password | String        | TLS 私钥密码。                                                |
| data.connector.ssl.keyfile      | String        | TLS 私钥数据。                                                |
| data.connector.ssl.certfile     | String        | TLS Cert 证书数据。                                           |
| data.connector.ssl.cacertfile   | String        | TLS CA 证书数据。                                             |
...
| data.quic_send_idle_timeout     | Duration      | QUIC 传输层重置拥塞控制算法的等待超时时间 (*默认`60 s`*)      |
| data.quic_initial_rtt_ms        | Duration      | 初始 RTT 估计时间 (*默认 `800ms`*)                           |
| data.quic_max_ack_delay_ms      | Duration      | 发送 ACK 之前接收数据后等待时长(默认`100ms`)                 |
| data.quic_qos_priority          | Boolean       | 高优先级发送 QOS 1 或 2 的消息(*默认 `true`*)                    |
...
| data.forwards                   | Array[String] | 转发 Topic 数组, 使用逗号`,`分隔多个`Topic`。                  |
