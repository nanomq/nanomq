...
- 当 MQTT 客户端向 `mqtt/local/#` 匹配的主题发布消息时，NanoMQ 会把配置的 `remote_topic` 作为前缀拼接到原始 payload 前面，形成 `remote_topic/payload` 格式的 NNG 原始消息。
...
- 当收到以 `test/123/` 为前缀的消息时，NanoMQ 会去掉此前缀，把剩余部分作为 MQTT payload，并发布到 `test/forward`。
...
- NNG 发送端应携带 `remote_topic/` 前缀，NanoMQ 会在转换时剥离此前缀。
...
5. `bridges.nng.sub` 测试时，NNG 消息是否以前缀 `test/123/` 开头，以及 MQTT 订阅主题是否为 `test/forward`。
