...
按 topic 与时间范围查询 parquet 数据记录。

**Parameters (query)：**

| Name     | Type    | Required | Description |
| -------- | ------- | -------- | ----------- |
| topic    | String  | True     | Exchange 主题名。 |
| start_ts | Integer | True     | 起始时间戳（包含边界），单位毫秒。 |
| end_ts   | Integer | True     | 结束时间戳（包含边界），单位毫秒。 |
| schema   | String  | False    | 逗号分隔的列名（例如：`data,can_id`）。不传时默认查询除 ts 外的所有列。 |
| limit    | Integer | False    | 最多返回记录条数。不传时默认值为 `10000`。`limit=0` 视为非法参数，返回 HTTP `400`。 |
| offset   | Integer | False    | 返回前跳过的匹配记录条数。默认值为 `0`。 |

**Success Response Body (JSON)：**

| Name                | Type             | Description |
| ------------------- | ---------------- | ----------- |
| count               | Integer          | 返回记录条数。 |
| dataArray           | Array of Objects | 查询结果数组。 |
| dataArray[0].ts     | Integer          | 时间戳（毫秒）。 |
| dataArray[0].data   | String           | 选定列拼接后的 Base64 编码数据。 |
