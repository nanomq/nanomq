...
class Test(object):
    ...
    def assert_test(self, send_times, _qos, sub, pub):
        ...
        # test sub
        while qos >= 0:
            self._mqttc = mqtt.Client(callback_api_version=CallbackAPIVersion.VERSION1, client_id="ws_sub_test", transport=self._tran, protocol=self._prot)   
        ...
        # test unsub
        while qos >= 0:
            self._mqttc = mqtt.Client(callback_api_version=CallbackAPIVersion.VERSION1, client_id="ws_unsub_test", transport=self._tran, protocol=self._prot)   
            # self._mqttc.on_connect = on_connect
            self._mqttc.on_message = on_message
            self._mqttc.on_publish = on_publish
            self._mqttc.on_log = on_log
