...
uint8_t decode_sub_message(nng_msg *msg, packet_subscribe *sub_pkt)
{
	...
	while (1) {
		...
		debug_msg("Length of topic: %d topic_node: %x %x. ", len_of_topic,
		          (uint8_t) (topic_option->topic_filter.str_body[0]),
		          (uint8_t) (topic_option->topic_filter.str_body[1]));
...
uint8_t encode_suback_message(nng_msg *msg, packet_subscribe *sub_pkt)
{
	...
	debug_msg("remain: [%d] varint: [%d %d %d %d] len: [%d] packet_id: [%x %x]", remaining_len, varint[0], varint[1],
	          varint[2], varint[3], len_of_varint, packet_id[0], packet_id[1]);
	return SUCCESS;
}
