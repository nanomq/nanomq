 ...
 */
static bool binary_search(void **vec, int l, int *index, void *e, int (*cmp)(void *x, void *y)) 
{ 
 ...
 */
static int skip_wildcard(db_node *node)
{
        int l = 0;
        if(node->plus != -1) {
                l++;
 ...
 * @return s_client*
 */
static s_client *new_db_client(char *id, void *ctxt, uint32_t pipe_id)
{
...
static void *insert_db_client(db_node *node, void *args)
{
        ...
        // printf("pipe_id is : %d\n", client->pipe_id);
        if (false == binary_search((void**)node->clients, 0, &index, /*client->id,*/ &(client->pipe_id), client_cmp)) {
...
// TODO 
// static wildcard_dealer(db_node *node, char **topic_queue, s_client *client, bool(*check(char *topic)) 
// {
 ...
 * @return void
 */
static db_node *insert_db_node(db_node *node, char **topic_queue)
{
                ...
                for (int i = 0; i < cvector_size(v); ++i) {
                        for (int j = 0; j < cvector_size(v[i]); j++) {
                                ...
                                if (equal == false) {
                                        ...
                                        //TODO  binary sort and cvector_insert();
 ...
 * @return 
 */
static void *delete_db_client(db_node *node/*, char *id*/, uint32_t pipe_id)
{
        int index = 0;
        void * ctxt = NULL;
        ...
        if (true == binary_search((void**)node->clients, 0, &index, &pipe_id, client_cmp)) {
 ...
 */
void *search_and_delete(db_tree *db, char *topic, /*char *id*/uint32_t pipe_id)
{
        assert(db->root && topic);
        pthread_rwlock_wrlock(&(db->rwlock));
...
retain_msg **collect_retain_well(retain_msg **vec, db_node *node)
{
        db_node **nodes = NULL;
        db_node **nodes_t = NULL;
        cvector_push_back(nodes, node);
 ...
 * @return all clients on lots of nodes
 */
static retain_msg **collect_retains(retain_msg **vec, db_node **nodes, db_node ***nodes_t, char **topic_queue)
{
