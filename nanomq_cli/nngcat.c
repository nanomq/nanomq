...
struct topic **
nng_addtopic(struct topic **endp, const char *s)
{
	struct topic *t;
...
nng_cat_dflt(int ac, char **av)
{
	...
	while ((rv = nng_opts_parse(ac - 1, av + 1, opts, &val, &arg, &idx)) ==
		...
		case OPT_COUNT:
			...
			break;
		case OPT_SUBSCRIBE:
			topicend = nng_addtopic(topicend, arg);
			break;
		case OPT_VERBOSE:
		case OPT_SILENT:
			verbose = val;
	...
	if (proto == OPT_SUB0) {
		if (topics == NULL) {
			(void) nng_addtopic(topicend, ""); // subscribe to all
		}
	} else {
		if (topics != NULL) {
